...
area2= [RichMenuArea(bounds= RichMenuBounds(x= 0, y= 0, ##左選單
                                            width= 1200, height= 185),
                     action= RichMenuSwitchAction(label= 'Switch_left', \
                                                  rich_menu_alias_id= 'richmenu-alias-left', data= "richmenu-changed-to-left")
                     ),
...
rich_right_menu = RichMenu(
    size= RichMenuSize(width= 2500, height= 1686),
    selected= True,
    name= 'rich_right_menu',
    chat_bar_text= '禮盒麻糬',
    areas= area2
    )

rich_right_menu_id = line_bot_api.create_rich_menu(rich_menu= rich_right_menu)
...
with open('temp.jpg', 'rb') as f:
    line_bot_api.set_rich_menu_image(rich_right_menu_id, 'image/jpeg', f)
    
alias_left = RichMenuAlias(
    rich_menu_alias_id='richmenu-alias-left',
    rich_menu_id= rich_left_menu_id
)
line_bot_api.create_rich_menu_alias(alias_left)

alias_right = RichMenuAlias(
    rich_menu_alias_id='richmenu-alias-right',
    rich_menu_id= rich_right_menu_id
)
line_bot_api.create_rich_menu_alias(alias_right)
...
def dscbot(event):
    ...
    flex = FlexSendMessage(
        ...
        contents = {
                  ...
                  "hero": {
                    "type": "image",
                    "url": "https://s3-ap-northeast-1.amazonaws.com/static.iyp.tw/37429/files/a9b19ebe-43e2-4887-b17b-7ac14a0a4b4e.png",
                    "size": "full",
                    "aspectRatio": "20:13",
                    "aspectMode": "cover",
                    "action": {
